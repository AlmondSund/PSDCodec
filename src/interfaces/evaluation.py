...
def load_demo_eval_report(
    report_path: str | Path,  # JSON report generated by `scripts/tools/demo_eval.py`
) -> RateDistortionComplexityReport:
    """Load a saved demo evaluation report back into typed dataclasses.

    Purpose:
        The notebook should analyze the generated report rather than rebuild the
        benchmark by default. This loader restores the JSON artifact into the same
        typed structure used by the evaluation interface.
    """
    payload = json.loads(Path(report_path).read_text(encoding="utf-8"))
    validation_reference = payload["validation_reference"]
    dataset = payload["dataset"]
    quality = payload["quality"]
    payload_cost = payload["payload"]
    runtime = payload["runtime"]
    complexity = payload["complexity"]
    return RateDistortionComplexityReport(
        export_dir=Path(payload["export_dir"]),
        checkpoint_path=Path(payload["checkpoint_path"]),
        onnx_provider=str(payload["onnx_provider"]),
        validation_reference=ValidationReferenceSummary(
            summary_path=Path(validation_reference["summary_path"]),
            best_epoch_index=int(validation_reference["best_epoch_index"]),
            psd_distortion_mean=float(validation_reference["psd_distortion_mean"]),
            preprocessing_distortion_mean=float(
                validation_reference["preprocessing_distortion_mean"]
            ),
            rate_proxy_bits_mean=float(validation_reference["rate_proxy_bits_mean"]),
            task_monitor_mean=(
                None
                if validation_reference["task_monitor_mean"] is None
                else float(validation_reference["task_monitor_mean"])
            ),
            deployment_score=(
                None
                if validation_reference["deployment_score"] is None
                else float(validation_reference["deployment_score"])
            ),
        ),
        dataset=EvaluationDatasetSummary(
            source_kind=cast(
                Literal["prepared_npz", "raw_campaigns"],
                str(dataset["source_kind"]),
            ),
            dataset_path=Path(dataset["dataset_path"]),
            evaluation_split=str(dataset["evaluation_split"]),
            total_frame_count=int(dataset["total_frame_count"]),
            evaluation_frame_count=int(dataset["evaluation_frame_count"]),
            runtime_frame_count=int(dataset["runtime_frame_count"]),
            original_bin_count=int(dataset["original_bin_count"]),
            reduced_bin_count=int(dataset["reduced_bin_count"]),
            block_count=int(dataset["block_count"]),
            excluded_campaign_labels=tuple(
                str(campaign_label) for campaign_label in dataset["excluded_campaign_labels"]
            ),
        ),
        quality=ReconstructionQualitySummary(
            psd_distortion_mean=float(quality["psd_distortion_mean"]),
            psd_distortion_std=float(quality["psd_distortion_std"]),
            psd_distortion_min=float(quality["psd_distortion_min"]),
            psd_distortion_max=float(quality["psd_distortion_max"]),
            preprocessing_distortion_mean=float(quality["preprocessing_distortion_mean"]),
            codec_distortion_mean=float(quality["codec_distortion_mean"]),
            task_distortion_mean=(
                None
                if quality["task_distortion_mean"] is None
                else float(quality["task_distortion_mean"])
            ),
        ),
        payload=PayloadCostSummary(
            operational_bits_mean=float(payload_cost["operational_bits_mean"]),
            operational_bits_std=float(payload_cost["operational_bits_std"]),
            operational_bits_min=int(payload_cost["operational_bits_min"]),
            operational_bits_max=int(payload_cost["operational_bits_max"]),
            side_information_bits_mean=float(payload_cost["side_information_bits_mean"]),
            index_bits_mean=float(payload_cost["index_bits_mean"]),
            rate_proxy_bits_mean=float(payload_cost["rate_proxy_bits_mean"]),
            bits_per_original_bin_mean=float(payload_cost["bits_per_original_bin_mean"]),
            bits_per_reduced_bin_mean=float(payload_cost["bits_per_reduced_bin_mean"]),
            bits_per_latent_index_mean=float(payload_cost["bits_per_latent_index_mean"]),
        ),
        runtime=RuntimeCostSummary(
            encode_latency_mean_ms=float(runtime["encode_latency_mean_ms"]),
            encode_latency_std_ms=float(runtime["encode_latency_std_ms"]),
            encode_latency_min_ms=float(runtime["encode_latency_min_ms"]),
            encode_latency_max_ms=float(runtime["encode_latency_max_ms"]),
            decode_latency_mean_ms=float(runtime["decode_latency_mean_ms"]),
            decode_latency_std_ms=float(runtime["decode_latency_std_ms"]),
            decode_latency_min_ms=float(runtime["decode_latency_min_ms"]),
            decode_latency_max_ms=float(runtime["decode_latency_max_ms"]),
            roundtrip_exact_fraction=float(runtime["roundtrip_exact_fraction"]),
        ),
        complexity=ModelComplexitySummary(
            total_parameter_count=int(complexity["total_parameter_count"]),
            trainable_parameter_count=int(complexity["trainable_parameter_count"]),
            encoder_parameter_count=int(complexity["encoder_parameter_count"]),
            vector_quantizer_parameter_count=int(complexity["vector_quantizer_parameter_count"]),
            decoder_parameter_count=int(complexity["decoder_parameter_count"]),
            entropy_model_parameter_count=int(complexity["entropy_model_parameter_count"]),
        ),
    )


def build_demo_eval_summary_rows(
    report: RateDistortionComplexityReport,
) -> tuple[dict[str, str | float | int], ...]:
    """Return notebook-friendly rows summarizing the key demo evaluation metrics."""
    ...
    return (
        {
            "section": "Validation",
            "metric": "Held-out PSD distortion",
            "value": report.validation_reference.psd_distortion_mean,
            "units": "D_psd",
        },
        {
            "section": "Validation",
            "metric": "Held-out preprocessing distortion",
            "value": report.validation_reference.preprocessing_distortion_mean,
            "units": "D_psd",
        },
        {
            "section": "Validation",
            "metric": "Held-out rate proxy",
            "value": report.validation_reference.rate_proxy_bits_mean,
            "units": "bits/frame",
        },
        {
            "section": "Benchmark",
            "metric": "Benchmark PSD distortion",
            "value": report.quality.psd_distortion_mean,
            "units": "D_psd",
        },
        {
            "section": "Benchmark",
            "metric": "Benchmark operational payload",
            "value": report.payload.operational_bits_mean,
            "units": "bits/frame",
        },
        {
            "section": "Benchmark",
            "metric": "Side-information share",
            "value": side_information_share_percent,
            "units": "%",
        },
        {
            "section": "Runtime",
            "metric": "Encode latency",
            "value": report.runtime.encode_latency_mean_ms,
            "units": "ms/frame",
        },
        {
            "section": "Runtime",
            "metric": "Decode latency",
            "value": report.runtime.decode_latency_mean_ms,
            "units": "ms/frame",
        },
        {
            "section": "Runtime",
            "metric": "Exact round-trip fraction",
            "value": report.runtime.roundtrip_exact_fraction,
            "units": "fraction",
        },
        {
            "section": "Complexity",
            "metric": "Total parameters",
            "value": report.complexity.total_parameter_count,
            "units": "parameters",
        },
        {
            "section": "Complexity",
            "metric": "Encoder parameters",
            "value": report.complexity.encoder_parameter_count,
            "units": "parameters",
        },
        {
            "section": "Complexity",
            "metric": "Decoder parameters",
            "value": report.complexity.decoder_parameter_count,
            "units": "parameters",
        },
    )


def create_demo_eval_figure(
    report: RateDistortionComplexityReport,
) -> Any:
    """Create a four-panel matplotlib figure summarizing the demo evaluation.

    Purpose:
        The notebook should present the benchmark as one coherent graphical
        overview rather than as a long list of scalar metrics. The panels expose:

        - distortion comparisons between held-out and benchmark boundaries,
        - payload composition and rate-proxy references,
        - encode/decode latency with host-side variability,
        - parameter allocation across the learned codec subsystems.
    """
    ...
    distortion_values = np.asarray(
        [
            report.validation_reference.preprocessing_distortion_mean,
            report.validation_reference.psd_distortion_mean,
            report.quality.preprocessing_distortion_mean,
            report.quality.psd_distortion_mean,
        ],
        dtype=np.float64,
    )
    distortion_bars = distortion_axis.bar(
        distortion_labels,
        distortion_values,
        color=["#BFC0C0", "#1B4965", "#A0C4B8", "#F4A259"],
    )
    ...
    runtime_means = np.asarray(
        [
            report.runtime.encode_latency_mean_ms,
            report.runtime.decode_latency_mean_ms,
        ],
        dtype=np.float64,
    )
    runtime_errors = np.asarray(
        [
            report.runtime.encode_latency_std_ms,
            report.runtime.decode_latency_std_ms,
        ],
        dtype=np.float64,
    )
    ...
    complexity_values_millions = (
        np.asarray(
            [
                report.complexity.encoder_parameter_count,
                report.complexity.decoder_parameter_count,
                report.complexity.vector_quantizer_parameter_count,
                report.complexity.entropy_model_parameter_count,
            ],
            dtype=np.float64,
        )
        / 1.0e6
    )
    complexity_bars = complexity_axis.bar(
        ["Encoder", "Decoder", "VQ", "Entropy"],
        complexity_values_millions,
        color=["#003049", "#669BBC", "#F77F00", "#D62828"],
    )
    ...
    figure.text(
        ...
        (
            ...
            f"Excluded campaigns: "
            f"{', '.join(report.dataset.excluded_campaign_labels) or 'none'}"
        ),
        ha="left",
        va="bottom",
        fontsize=9.5,
...
def _require_matplotlib() -> Any:
    """Return the imported matplotlib.pyplot module or raise a clear error."""
    if _plt is None:
        raise ImportError(
            "matplotlib is required for graphical demo evaluation analysis.",
        )
    return _plt


def _annotate_bar_values(
    axis: Any,  # Matplotlib axis receiving the label annotations
    bars: Any,  # Iterable of bar artists created by `Axes.bar`
    *,
    value_format: str,
) -> None:
    """Write numeric labels slightly above each bar."""
    axis.set_axisbelow(True)
    y_min, y_max = axis.get_ylim()
    y_offset = 0.02 * (y_max - y_min if y_max > y_min else 1.0)
    for bar in bars:
